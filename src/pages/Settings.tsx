...
export const Settings: React.FC = () => {
  ...
  const handleLanguageChange = (lang: Language) => {
    ...
    speak(`System language updated to ${
      lang === 'en' ? 'English' : lang === 'es' ? 'Spanish' : lang === 'fr' ? 'French' : 'Arabic'
    }`);
  };
  ...
  const handleToggle = <K extends keyof typeof settings>(key: K, name: string) => {
    ...
    speak(`${name} ${nextVal ? 'enabled' : 'disabled'}`);
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">
          Manage system configurations, user profiles, and operations alerts
        </p>
      </div>
          ...
          <CardContent className="p-6 space-y-5 text-left text-xs font-semibold">
            ...
            <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-100 space-y-3">
              <h4 className="text-sm font-bold text-primary dark:text-white flex items-center gap-2 uppercase tracking-wide">
                ...
                Tournament System Language
              </h4>
              <p className="text-slate-500 dark:text-slate-400 font-medium">Select your preferred FWC stadium documentation language translation.</p>
            ...
            <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-100 space-y-4">
              <h4 className="text-sm font-bold text-primary dark:text-white flex items-center gap-2 uppercase tracking-wide">
                ...
                Telemetry Banners & Audio
              </h4>

              <div className="space-y-3">
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-2.5">
                  <div>
                    <span className="font-bold">Real-time Push Notifications</span>
                    <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">Surge queue and transit notifications</p>
                  </div>
                  <input
                    type="checkbox"
                ...
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-2.5">
                  <div>
                    <span className="font-bold">Sound Alerts Ticker</span>
                    <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">Goal scores sound cues and emergency notices</p>
                  </div>
                  <input
                    type="checkbox"
                ...
                <div className="flex items-center justify-between pb-1">
                  <div>
                    <span className="font-bold">In-Seat Delivery Alerts</span>
                    <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">Receive notifications when volunteer courier leaves concession</p>
                  </div>
                  <input
                    type="checkbox"
        ...
        <Card className="border border-slate-100 shadow-md">
          ...
          <CardContent className="p-5 space-y-4 text-xs font-semibold text-left">
            <p className="text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
              Organizers can simulate evacuation safety exercises in this companion portal to check volunteer task coordination.
            </p>

            <div className="space-y-4 pt-2">
              <div className="p-3 bg-red-50/30 dark:bg-red-950/10 border border-red-200/50 rounded-xl space-y-2">
                <span className="font-extrabold text-red-600 dark:text-red-400 uppercase">Stadium Evacuation Drill</span>
                <p className="text-[11px] text-slate-600 dark:text-white/60 leading-normal font-medium">Toggling this globally broadcasts evacuation banners to all Fan digital wallets, directing them to the nearest exit gate.</p>
                
                <Button 
                  onClick={() => {
                    const nextVal = !settings.evacuationMode;
                    updateSetting('evacuationMode', nextVal);
                    speak(nextVal ? "Evacuation drill active." : "Evacuation drill terminated.");
                  }}
                  variant={settings.evacuationMode ? 'primary' : 'danger'}
                  fullWidth
                  className="text-[10px] tracking-wider font-extrabold uppercase mt-1"
                >
                  {settings.evacuationMode ? 'TERMINATE DRILL' : 'TRIGGER EVAC SIGNALS'}
                </Button>
              </div>
            </div>
          </CardContent>
