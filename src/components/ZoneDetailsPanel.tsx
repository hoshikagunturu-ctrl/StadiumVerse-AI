...
export const ZoneDetailsPanel: React.FC<ZoneDetailsPanelProps> = ({ selectedZone }) => {
  if (!selectedZone) {
    return (
      <DashboardCard title="Zone Telemetry" variant="glass" className="h-full border border-slate-100 dark:border-white/10 shadow-md bg-white dark:bg-stadium-dark">
        <div className="flex flex-col items-center justify-center py-20 text-center text-slate-400 space-y-4">
          ...
          <div>
            <h4 className="font-extrabold text-xs uppercase tracking-wider text-slate-600 dark:text-slate-300">No Zone Selected</h4>
            <p className="text-[10px] font-medium max-w-xs mt-1">Click any standalone sector zone inside the digital twin layout map to check live operational telemetry parameters.</p>
          </div>
        </div>
      </DashboardCard>
    );
  ...
  return (
    <DashboardCard 
      title={`Twin Telemetry • ${selectedZone.name}`} 
      variant="glass" 
      ...
      <div className="space-y-4 text-xs font-semibold">
        ...
        <div className="flex justify-between items-center bg-slate-50/50 dark:bg-white/5 p-3 rounded-xl border border-slate-100 dark:border-white/5">
          ...
          <div className="text-right">
            <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">Risk Level</span>
        ...
        <div className="space-y-3.5 pt-2">
          {[
            { label: "Occupancy Capacity", val: `${selectedZone.occupancy} of ${selectedZone.capacity}`, icon: <Users size={14} className="text-secondary" /> },
            { label: "Active Deployments", val: `${selectedZone.staffCount} Officers / Stewards`, icon: <Shield size={14} className="text-accent" /> },
            { label: "Entry Speed Rates", val: selectedZone.entryRate, icon: <Activity size={14} className="text-blue-400" /> },
            { label: "Active Incidents Logged", val: `${selectedZone.incidentCount} incidents`, icon: <Shield size={14} className="text-red-500" /> }
          ].map((row, idx) => (
            <div key={idx} className="flex justify-between items-center text-xs border-b border-slate-50 dark:border-white/5 pb-2">
              <div className="flex items-center gap-2">
                {row.icon}
                <span className="text-slate-400 uppercase text-[9px] tracking-wider font-extrabold">{row.label}</span>
              </div>
